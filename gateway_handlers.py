...
async def handle_db_query(request):
    ...
    """
    # Main script (__main__) ke live global variables ko directly target karna
    import sys
    ...
    # Agar engine abhi tak initialize nahi hua ya variables nahi mile
    ...
    try:
        # Request body parse karna orjson ke saath
        ...
        if not is_readonly_sql(sql_query):
            return web.json_response({"error": "Security Block: Only single SELECT or WITH queries are allowed"}, status=400)

        # Thread-safe execution using main engine's DB_LOCK
        with db_lock:
            cursor = db_conn.execute(sql_query, params)
            columns = [col[0] for col in cursor.description] if cursor.description else []
            # Resource management: Max 10,000 rows limit tak fetch karenge
        ...
        # Orjson se super-fast response serialize karke bhej rahe hain
        return web.Response(
            body=orjson.dumps({
                "db": db_filename,
                "count": len(rows),
