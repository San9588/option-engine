...
export default function OptionChainPage() {
  ...
  // Keep strikeRange in a ref so the WebSocket effect doesn't need to
  // depend on it (avoids reconnect on range change — filtering is client-side).
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        ...
        // Subscribe with FULL range (-30 to +30) — client-side filtering by strikeRange
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (event.data instanceof ArrayBuffer) {
            ...
            // Client-side filtering: only show rows within strikeRange
            const range = strikeRangeRef.current;
            const parsedRows = tick.data.filter((row: OptionRow) =>
              Math.abs(row.relative_idx) <= range
            );
  ...
  }, [wsUrl, symbol, settingsLoaded]); // strikeRange NOT here — filtering is client-side

  // When strikeRange changes, filter existing rows (no WS reconnect needed)
  useEffect(() => {
    setRows(prev => prev.filter(row => Math.abs(row.relative_idx) <= strikeRange));
  }, [strikeRange]);
